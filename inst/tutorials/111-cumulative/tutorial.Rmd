...
plot_predictions(fit_approval, 
                 newdata = "balanced", 
                 condition = c("education", "approval", "ideology"))

plot_predictions(fit_approval, 
                 newdata = "balanced", 
                 condition = c("education", "ideology"),
                 draw = FALSE) -> z
```
...
<!-- DK: Should we drop ideology = "Not Sure" from the data? -->
<!-- DK: Equitomatic does not work? How do we deal with the math? -->
...
# MP The function Show_file() from the tutorial.helpers package is mentioned in this exercise, but the package itself is not called. As a result, students may struggle to answer the question. To address this, consider including a question that explicitly requires the use of Show_file() to ensure students are aware of how to use the function and the package.
...
# The transition between questions is quite abrupt and unclear. It would be helpful to provide more context or refer back to the previous question about election approval ratings. This can be achieved by adding more reference to the elections data.
...
## MP The tutorial introduces a treatment in the context of ads earlier, but it should be clarified that the ads are not considered the treatment in the analysis. This clarification will help prevent confusion about the role of ads in the context of the exercise.
...
# MP The wording of this exercise is somewhat confusing and could be made clearer. I would suggest revising the question to improve response accuracy.
...
Add `library(broom)` to the `setup` code chunk in `analysis.qmd`. Copy and paste the below code for the mathematical structure of the model to the body of `analysis.qmd`. `Command/Ctrl + Shift + K`. 
...
```
tutorial.helpers::show_file("analysis.qmd", pattern = "broom|\\$\\$")
```
...
Create a new code chunk in `analysis.qmd`. Add two code chunk options: `label: model` and `cache: true`. Copy/paste the code from above for estimating the model using `polr()` into the code chunk, assigning the result to `fit_approval`.  `Command/Ctrl + Shift + K`. 
...
# MP An error occurs where the object X cannot be found. Additionally, the varying questions across exercises create some confusion about the central question, as seen in Exercise 3 from the Temperance section. Clarify the central question and ensure consistency in the exercises to provide a clearer focus.
...
Write a sentence interpreting the `-0.19` estimate for `educationHighSchoolGraduate`.
...
Write a sentence interpreting the `-0.29` to `-0.08` confidence interval for `educationHighSchoolGraduate`. 
...
```{r courage-9}
question_text(NULL,
	message = "We do not know the true value for the coefficient for educationHighSchoolGraduate, but we can be 95% confident that it lies somewhere between -0.29 and -0.08.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
Write a sentence interpreting the `5.43` estimate for `ideologyVeryConservative`.
...
```{r courage-10}
question_text(NULL,
	message = "If we compare people who are very conservative with people who are very liberal (the base category which is included in the intercept), the very conservative people approve of the president more. Because of the complexity of the mathematics of the model, 5.43 does not have a natural interpretation in terms of its magnitude.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
To answer our question, we need to create an object --- call it `ndata` --- which we will pass in as a value to the `ndata` argument in whichever **marginaleffects** function we decide to use. Which variables (e.g., which columns) do we need to include in this object?
...
Here is the R code which creates the `ndata` object: `expand_grid(ideology = c("Very Liberal", "Very Conservative"), education = unique(x$education), faminc = unique(x$faminc))`. Type it into the code exercise block and hit "Run Code."
...
expand_grid(ideology = factor(c("Very Liberal",
                          "Very Conservative")),
            education = unique(x$education),
...
The `ndata` tibble has 144 rows. There are 12 values for `faminc`, 2 values for `ideology`, and 6 values for `education`. The number of values for each of them multiplied together gives us 144. 

### Exercise 9

Pipe `fit_approval` to `add_predicted_draws()` with the argument `newdata = ndata`. Then continue the pipe with `filter(.prediction %in% c("Strongly Approve", "Strongly Disapprove"))`
...
The code below creates a nice graph. Click run. 
...
We have narrowed down our focus to only `Strongly Disapprove` and `Strongly Approve` so that we can compare the two. 

For `Strongly Disapprove`, the probability increases as the level of `education` increases. On the other hand, for `Strongly Approve`, the probability decreases as the level of `education` increases.
...
# MP Temperance Model Compilation Issue: When trying to compile the example model, the code snippet fit_approval |> add_predicted_draws(newdata = ndata) |> filter(.prediction %in% c("Strongly Approve", "Strongly Disapprove")) results in an error indicating that add_predicted_draws cannot be found. This suggests that the marginaleffects package may not be loaded correctly. Ensure that the package is properly loaded and that the function is accessible. 
