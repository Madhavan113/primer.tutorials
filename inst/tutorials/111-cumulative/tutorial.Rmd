...
<!-- DK: Create three questions which ask the students to run the code, in the Console, for creating simple marginaleffects plots, probably with plot_predictions(). Also, explore other plots. Maybe we should use one? Recall that the knowledge drop should be interpret the plot with three excellent sentences. -->
...
<!-- DK: Change so that we tell them about the command, tell them we have already created the object behind the scenes, and then tell them to just print out the object because it takes a minute to create. Then, in the QMD question, make sure they do it with cache = TRUE, and then use that object to be printed out in the next chunk. -->
...
```{r}
# results |> 
#   select(group, estimate, ideology)
```

```{r}
# results |> 
#   select(group, estimate, ideology, conf.low, conf.high) |> 
#   ggplot(aes(x = ideology, y = estimate, fill = group)) +
...
<!--MP: new marginal effects plot exercises

Let's try another plot, enter this code into the exercise code block and hit "run code"
...
plot_predictions(fit_approval,
                 condition = "ideology",
                 newdata = data.frame(education = "4-Year"))
Interpretation: Beta

plot_predictions(fit_approval, condition = )
...
Let's, prepare the dataset by ensuring the group column is a factor with appropriate levels. This step is essential for proper grouping in the plot. Set a variable `temp`
...
```{r temperance-13-hint-1, eval = TRUE}
ggplot(temp, aes(x = ideology, y = estimate, fill = group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", estimate)), position = position_stack(vjust = 0.5), size = 3) +
  coord_flip() +
